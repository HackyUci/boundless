...
export const ChatbotSection: React.FC = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_AI_URL}/chatbot/chat`,
        {
          method: "POST",
  ...
  const clearConversation = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_AI_URL}/chatbot/clear`,
        {
          method: "POST",
        }
      );
      ...
      if (response.ok) {
        ...
        addMessage(
          "assistant",
          "Conversation cleared! How can I help you with your study abroad planning?"
        );
      }
    } catch (error) {
        ...
        <CardContent className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-shrink-0 border-t pt-4">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Ask about universities, scholarships, preparation steps, or
              comparisons
            </p>
          </div>
        </CardContent>
