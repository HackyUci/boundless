...
export const AnalyzeResultSection = () => {
  ...
  const TimelineComponent = ({ steps }: { steps: PrepStep[] }) => (
    <div className="space-y-6">
      {steps.map((step, index) => (
        <div key={index} className="relative flex items-start space-x-4">
          <div className="flex flex-col items-center">
            <div className={`${getPriorityColor(step.priority)} w-10 h-10 rounded-full flex items-center justify-center border-2`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="flex justify-between items-center text-sm text-gray-600">
                ...
                <span className="font-medium">{formatCurrency(step.cost_idr)}</span>
              </div>
            </div>
          </div>
        </div>
  ...
  const TimelineContent = () => (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          {analysisData?.preparation_steps && (
            <TimelineComponent steps={analysisData.preparation_steps} />
          )}
        </CardContent>
      </Card>
    </div>
